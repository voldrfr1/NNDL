...
class MSE():
    ...
    def delta_error(output,labels,z):
        ...
        # analytically obtained
        return (output-labels)*sigmoid.derivative(z)  # can be multiplied by 2, but makes no change
...
class Network():
    ...
    def __init__(self, structure,cost=CrossEntropy,activation_functions=relu):
       ...
       ever used in computing the outputs from later layers."""
        ...
        self.bias_velocities = [np.zeros((b, 1)) for b in structure[1:]]
        self.weight_velocities=[np.zeros((y, x)) for x, y in zip(
            self.structure[:-1], self.structure[1:])]
        self.biases = [np.random.standard_normal((b, 1)) for b in structure[1:]]
        self. __weights_initializer()
        self.cost=cost
        self.grads=[[] for _ in range(self.num_layers-2)]
        self.epoch_grads=[[] for _ in range(self.num_layers-2)]
        if type(activation_functions) is not list: 
            activation_functions=[activation_functions for _ in range(self.num_layers-2)]
        self.activation_functions=activation_functions
        
    def __weights_initializer(self):
        """Initializes weights using N(0,1/sqrt(number of neurons in previous layer))."""
        ...
        self.weights = [np.random.normal(0,1/np.sqrt(x),(y, x)) for x, y in zip(
            self.structure[:-1], self.structure[1:])]
    ...
    def feedforward(self, X):
        ...
        for b, w,act in zip(self.biases[:-1], self.weights[:-1],self.activation_functions):
            ...
            X = act.fun(np.dot(w, X)+B)
        B=np.tile(self.biases[-1],(1,batch_size))
        X = sigmoid.fun(np.dot(self.weights[-1], X)+B)
        return X
    ...
    def fit(self, training_data, batch_size=16, max_epochs=10,early_stop_after=10, learning_rate=3,momentum_coeff=0,
            lmbda=0,total_halving=8, validation_data=None,monitor_learning=False):
        ...
        for i in range(max_epochs):
            ...
            for j in (pbar:=tqdm(range(0, N, batch_size))):
                ...
                self.__SGD(batch_X,batch_Y, eta,mu,lmbda,N)
            for l in range(self.num_layers-2):
                self.epoch_grads[l].append(np.mean(self.grads[l]))
            print(f"Epoch {i+1}/{max_epochs} completed.")
        ...
        print(f"Elapsed time for one epoch: {el_time/i:.4f} s.")
        return training_loss,training_accuracy,validation_loss,validation_accuracy

    def __SGD(self, X,Y, eta,mu,lmbda,N):
        ...
        nabla_b,nabla_w=self.__backpropagation(X,Y)
        ...
        self.bias_velocities = [mu*vb-(eta/batch_size)*delta_b
                       for vb, delta_b in zip(self.bias_velocities, sum_nabla_b)]
        self.weight_velocities = [mu*vw-eta/batch_size*(delta_w+lmbda/N*w)
                        for vw, delta_w,w in zip(self.weight_velocities, nabla_w,self.weights)]
        # update weights and biases according to SGD rule
        self.biases = [b+vb for b,vb in zip(self.biases, self.bias_velocities)]
        #This is where we perform the regularization
        self.weights = [w+vw for w, vw in zip(self.weights, self.weight_velocities)]
    ...
    def __backpropagation(self, X,Y):
        ...
        for b, w,act in zip(self.biases[:-1], self.weights[:-1],self.activation_functions):
            ...
            Z_arr.append(Z)
            activation = act.fun(Z)
            activation_arr.append(activation)
        B=np.tile(self.biases[-1],(1,batch_size))
        Z = np.dot(self.weights[-1], activation)+B
        Z_arr.append(Z)
        activation = sigmoid.fun(Z)
        activation_arr.append(activation)
        # backwardpass
        ...
        for l in range(2, self.num_layers):
            ...
            error = np.dot(
                self.weights[-l+1].transpose(), error)*self.activation_functions[-l+1].derivative(Z_arr[-l])
            # compute the gradients using formula
            nabla_b[-l] = error
            nabla_w[-l] = np.dot(error, activation_arr[-l-1].transpose())
            grad=np.sum(np.abs(error))
            self.grads[-l+1].append(grad)
