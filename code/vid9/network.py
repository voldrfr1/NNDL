...
class Network():
    ...
    def evaluate(self, test_data,batch_size=32,use_tqdm=True):
        """Computes loss and accuracy against the test_data.
        ...
        for j in pbar:
            ...
            loss=self.cost.loss(output,batch_Y)
    ...
    def fit(self, training_data, batch_size=16, epochs=10, learning_rate=3, 
            validation_data=None,monitor_learning=False):
        ...
        for i in range(epochs):
            ...
            for j in (pbar:=tqdm(range(0, N, batch_size))):
                ...
                if monitor_learning:
                    ...
                    loss,accuracy=self.evaluate((X[j:j+batch_size],Y[j:j+batch_size]),use_tqdm=False)
