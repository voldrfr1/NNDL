...
class Network():
    ...
    def fit(self, training_data, batch_size=16, max_epochs=10,es=10, learning_rate=3,
            lmbda=0,lr_halved=8, validation_data=None,monitor_learning=False):
        ...
        for i in range(max_epochs):
            ...
            for j in (pbar:=tqdm(range(0, N, batch_size))):
                ...
                self.__SGD(batch_X,batch_Y, learning_rate,lmbda,N)
            ...
            if validation_data:
                ...
                if len(validation_accuracy)>es:
                    ...
                    if np.mean(validation_accuracy[-es:])<np.mean(validation_accuracy[-2*es:-es]):
                        learning_rate/=2
                        halved+=1
                        print(f"Learning rate halved {halved}/{lr_halved} times.")
                    if halved>=lr_halved: 
                        print(f"Early stopped. No improvement in {es} epochs.")
                        break;
    ...
    def __SGD(self, X,Y, learning_rate,lmbda,N):
